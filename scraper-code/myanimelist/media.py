...
class Media(Base):
  ...
  def parse_sidebar(self, media_page):
    ...
    try:
      ...
      num_users = int(score_tag.find_all('span')[2].text.replace(',',''))
  ...
  def parse_recommendations(self, recommendations_page):
    ...
    media_info = self.parse_sidebar(recommendations_page)

    media_info[u'recommendations'] = []

    tr_tags = recommendations_page.findAll(u'tr')
    rec_list = []
    for tag in tr_tags:
        rec_list.append(tag.findAll("div", { "class": "picSurround"}))
    rec_list = filter(None, rec_list)

    # Just exctract the link part of the rec, in form of /type/id/title
    rec_list = [item[0].findAll('a', href=True)[0]['href'] for item in rec_list]

    # Eliminate weird duplicates while preserving order
    rec_list = [x[0] for x in itertools.groupby(rec_list)]

    # Get the number of users per recommendation
    rec_count = recommendations_page.findAll('a', {'class': 'js-similar-recommendations-button'})
    rec_count = [int(x.findAll('strong')[0].text) for x in rec_count]

    for i in range(0, len(rec_list)):
      if i >= len(rec_count):
        users_rec = 1
      else:
        users_rec = rec_count[i]
      # Extract and store media id along with number of users that made the rec
      media_info[u'recommendations'].append((rec_list[i], users_rec))
  ...
  def load_recommendations(self):
    ...
    """
    recommendations_page = self.session.session.get(u'http://myanimelist.net/' + self.__class__.__name__.lower() + u'/' + str(self.id) + u'/' + utilities.urlencode(self.title) + u'/userrecs').text
    self.set(self.parse_recommendations(utilities.get_clean_dom(recommendations_page)))
    return self
